...
genes_to_highlight <- c(
  ...
  "Selp", "Clu", "Vwf", "Slamf1", "Mecom", "Egr1", 
...
gene_categories <- list(
  "Stemness_Quiescence" = c("Selp", "Clu", "Vwf", "Slamf1", 
                            "Mecom", "Egr1", "Ndn", "Cdkn1b", "Hoxb5", "Fgd5"),
...
stopifnot(exists("gse_kegg_all"), exists("target_pathways"))

# Pull the full KEGG GSEA table and keep ONLY the target pathways that are
# activated (NES > 0). This drops Ras, MAPK, Rap1, Hematopoietic cell lineage
# and Th17 cell differentiation automatically.
bar_df <- as.data.frame(gse_kegg_all) %>%
  dplyr::filter(Description %in% target_pathways, NES > 0) %>%
  dplyr::mutate(
    Direction   = "Activated in Tet2-KO",
    Description  = fct_reorder(Description, NES)   # highest NES at the top
  )

cat("\nPathways kept in bar chart (", nrow(bar_df), "):\n", sep = "")
print(as.character(levels(droplevels(bar_df$Description))))
...
p_bar <- ggplot(bar_df,
  ...
  scale_x_continuous(
    limits = c(0, ceiling(max(bar_df$NES) * 10) / 10 + 0.2),
    expand = expansion(mult = c(0, 0.02))
  ) +
  labs(
    title    = "KEGG GSEA \u2014 Target Pathways",
    subtitle = "Tet2-KO vs WT LSK Cells | Ranked by DESeq2 Wald statistic",
    x        = "Normalized Enrichment Score (NES)",
    y        = NULL
  ) +
  theme_classic(base_size = 12) +
...
ggsave(file.path(output_folder, "GSEA_KEGG_Activated_BarChart_New.png"),
       plot = p_bar, width = 9, height = 5, dpi = 300)
cat("Saved: GSEA_KEGG_Activated_BarChart_NEW.png (downregulated removed, no NES/FDR labels)\n")
